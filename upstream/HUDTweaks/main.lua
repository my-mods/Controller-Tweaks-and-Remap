...
local function WriteState(entry, target)
...
-- full object name.
local function Visit(widget, parentName, depth, seen, rootKey)
...
local function WalkWidget(widget, parentName, depth, seen, rootKey)
	if depth > S.maxDepth then return end
	if #discovered >= S.maxWidgets then return end

	local entry = Visit(widget, parentName, depth, seen, rootKey)
	if entry == nil then return end

	for _, child in ipairs(ChildrenOf(widget)) do
		WalkWidget(child, entry.name, depth + 1, seen, rootKey)
	end
end
...
local function CollectSubtree(root, list, seen, depth)
	if depth > S.maxDepth then return end
	...
	for _, child in ipairs(ChildrenOf(root)) do
		CollectSubtree(child, list, seen, depth + 1)
	end
end
	...
	for _, item in ipairs(list) do
		...
		if claims ~= nil then
			...
			if target ~= nil then
				...
				if item.key ~= rootFullName then lowered = NameKey(item.name) end
				byName[lowered] = {
					target   = target,
...
local function TweakInstance(root, specKey)
	...
	for _, item in ipairs(items) do
		...
		if item.key ~= rootFullName then lookup = NameKey(item.name) end
...
local function PlayerPawn()
	if IsValidObject(fadePlayer) and not IsDefaultObject(fadePlayer) then return fadePlayer end
	fadePlayer = nil

	local ok, list = pcall(FindAllOf, PLAYER_CLASS)
	if not ok or list == nil then return nil end

	local possessed = nil
	local pc = FirstLiveOf("PlayerController")
	if pc ~= nil then
		pcall(function()
			local pawn = pc.Pawn
			if IsValidObject(pawn) then possessed = SafeName(pawn) end
		end)
	end
	if possessed == nil then return nil end

	for _, obj in pairs(list) do
		if IsValidObject(obj) and not IsDefaultObject(obj) and SafeName(obj) == possessed then
			fadePlayer = obj
			return fadePlayer
		end
	end
...
local function AutoFadeStep()
	...
	if fadeNextProbe == nil or now >= fadeNextProbe then
...
local function AutoFadeHeartbeat(generation)
	...
	local delay = Differs(fadeLevel, fadeWant) and S.fadeStepMs or S.autoFadeMs
...
local function Heartbeat(generation)
	...
	ExecuteWithDelay(S.reassertMs, function()
		if generation ~= applyGeneration then return end
...
local function ForceHeartbeat(generation)
	...
	ExecuteWithDelay(S.forceReassertMs, function()
		if generation ~= applyGeneration then return end
...
local function SweepHeartbeat(generation)
	...
	ExecuteWithDelay(S.sweepMs, function()
		if generation ~= applyGeneration then return end
...
local function MenuSweepHeartbeat(generation)
	...
	ExecuteWithDelay(S.menuSweepMs, function()
		if generation ~= applyGeneration then return end
		if S.enabled and not suspended then
...
local function IdentifyBlink(generation)
	...
	ExecuteWithDelay(S.identifyMs, function()
		if generation ~= applyGeneration then return end
			...
			local ok = pcall(function()
				...
				NotifyOnNewObject(spec.path, function(obj)
					...
					for _, seconds in ipairs(S.newObjectFollowUps or {}) do
						local delay = math.floor(seconds * 1000)
						ExecuteWithDelay(delay, function() pcall(TweakInstance, obj, specKey) end)
					end
				end)
			end)
...
RegisterHook("/Script/Engine.PlayerController:ClientRestart", function()
	...
	if not beginPlayHooked then
		beginPlayHooked = pcall(function()
			RegisterHook(BEGIN_PLAY, function()
				...
				PurgeDead()
				ExecuteWithDelay(2000, function() ApplySequence("begin play") end)
			end)
		end)
	...
	ExecuteWithDelay(4000, function() ApplySequence("client restart") end)
end)
...
if S.startImmediately then
	...
	for _, ms in ipairs({ 50, 100, 250, 500, 1000, 2000, 4000 }) do
		ExecuteWithDelay(ms, function() pcall(PatchClassDefaults) end)
	end
end
