...
-- Uses reflected input APIs and Mod Settings snapshots; never saves game settings.
...
-- survive a callback. At most 8 steps and one rebuild per engine frame;
...
local function step()
    ...
    elseif job.phase == 'preset' then
        ...
        local mappings=state.preset.PresetMapping
        local preset, action = state.preset, change.action
        Session.change('preset:'..tostring(preset:GetAddress())..':'..action, function()
            if not live(preset) then return nil, false end
            local map, id = preset.PresetMapping, FName(action)
            return map:Contains(id) and keyName(unwrap(map:Find(id)).Key) or false
        end, function(value)
            local map, id = preset.PresetMapping, FName(action)
            if value == false then map:Remove(id)
            else map:Add(id,{Key={KeyName=FName(value)}}) end
        end, change.desired)
    ...
    elseif job.phase == 'profileApply' then
        ...
        local changed=Session.change('profile:'..tostring(profile.profile:GetAddress())..':'..alias, function()
            if not live(profile.profile) or not live(profile.settings) then return nil, false end
            return ControllerProfile.inspect(profile,alias,'Gamepad_FaceButton_Bottom')
        end, function(value)
            ControllerProfile.apply(profile,alias,value)
    ...
    elseif job.phase == 'mapping' then
        ...
        if current:sub(1,8)=='Gamepad_' then
            ...
            if target and current~=target.desired then
                ...
                local inputLibrary = library
                local contextId = context:GetAddress()
                local restoration = restoreContexts[contextId]
                if not restoration or not live(restoration.object) then
                    restoration = {object=context, dirty=false}
                    restoreContexts[contextId] = restoration
                    Session.onClose(function()
                        if restoration.dirty and live(context) and live(inputLibrary) then
                            inputLibrary:RequestRebuildControlMappingsUsingContext(context,false)
                            restoration.dirty = false
                        end
                    end)
                end
                Session.change('context:'..tostring(context:GetAddress())..':'..tostring(address)..':'..index, function()
                    if not live(context) or not live(inputLibrary) then return nil, false end
                    local rows = context.Mappings
                    if #rows ~= count or rows:GetArrayDataAddress() ~= address then return nil, false end
                    local row = at(rows,index)
                    if name(inputLibrary:GetMappingName(row)):lower() ~= alias then return nil, false end
                    return keyName(row.Key)
                end, function(value)
                    at(context.Mappings,index).Key={KeyName=FName(value)}
                    if not Session.active then restoration.dirty = true end
                end,target.desired)
...
tick = function()
    ...
    local ok,err=pcall(function()
        ...
        if not validState(not ownerChecked) then
            ...
            state,job,requested=nil,nil,{};running=false;return
        end
        ownerChecked=true
...
local hooksOK,hookError=pcall(function()
    ...
    hook('/Script/EnhancedInput.EnhancedInputSubsystemInterface:AddMappingContext',function(context,mappingContext)
        ...
        if exhausted then
            ...
            recoveryContext=id;exhausted=false
        end
    ...
    NotifyOnNewObject('/Script/RebelInput.RebelInputMappingSubsystem',function(object)
        ...
        if not state then exhausted=false end
        wake()
    end)
